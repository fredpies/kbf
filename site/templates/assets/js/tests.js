  ...
  var KbfAddressAutocomplete = /*#__PURE__*/function (_EventTarget) {
    ...
    function KbfAddressAutocomplete(selector) {
      ...
      _this.config = _objectSpread({
        ...
        formatResult: function formatResult(item) {
          ...
          instance.emitAddressChange(); // instance.emit(new CustomEvent('address-change', { detail: instance.currentAddressInfo }))

          return {
            id: item.place_id,
            text: item.address.road + ' ' + item.address.house_number,
            html: "".concat(item.address.road.toUpperCase() + ' ' + item.address.house_number.toUpperCase(), ", <b>").concat(item.address.city.toUpperCase(), "</b>")
          };
        },
        events: {
          ...
          searchPre: function searchPre(query) {
            ...
            var isQueryValid = new RegExp("[A-Za-zŃÓŻŹŁŚńóżźłś]+\\s+\\d+[a-zA-Z]?").test(query);
    ...
    _createClass(KbfAddressAutocomplete, [{
      ...
      value: function init() {
        ...
        new Inputmask$1().mask(this.$addressAutocomplete[0]);
